...
export default function AdminBoard(props) {
  ...
  useEffect(() => {
    ...
    axios.get(COMPANY_URL).then(({ data }) => {
      setCompany(data);
    });
  }, []);
  ...
  const removeComp = e => {
    const id = e.target.id;
    axios.delete(`${COMPANY_URL}/${id}`).then(res => {
      const filtered = users.filter(el => el.id !== id);
      setCompany(filtered);
    });
  };
            ...
            {menuItem === "users" ? (
              <Table dataSource={users}>
                ...
                <Column
                  ...
                  key="passportURL"
                  render={(text, record) => (
                    <img style={{height:"20px"}} src={record.passportURL}></img>
                ...
                <Column
                  title="Approved"
                  key="approved"
                  render={(text, record) => (record.approved ? "Yes" : "No")}
                />
                <Column
                  title="Accept"
                  key="accept"
                ...
                <Column
                  ...
                  render={(text, record) => (
                    <span>
                      <Button type="danger" id={record.id} onClick={remove}>
                        Delete
                      </Button>
                    </span>
                  )}
              ...
              <Table dataSource={companies}>
                ...
                <Column
                  ...
                  render={(text, record) => (
                    <span>
                      <Button type="danger" id={record.id}  onClick={removeComp}>
                        Delete
                      </Button>
                    </span>
                  )}
